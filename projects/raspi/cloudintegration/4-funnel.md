...
- `payloads` 要素にデバイスから送信したJSONデータが格納されます
- 他の要素はFunnelによって追加された、SIMや送信データに関するメタデータです
- `timestamp`にはFunnelがデータを受信した時刻をUNIXタイムスタンプ形式で入るので、デバイスで時刻が取得できない場合などに利用することができます
- `operatorID`にはSORACOMのアカウントIDが入るので、複数のユーザーのマルチテナント環境で識別子として利用できます
- `imsi`にはIoT SIMの識別子(IMSI)が入るので、どのデバイスから来たデータなのかを識別する情報として利用できます

Funnelのデータ形式を変更し、その様子を見てみましょう。SORACOM Webコンソールのグループ設定画面を開き、Funnelの設定のうち [送信データ形式] を「テキスト」に変更し[保存]ボタンをクリック、Raspberry Piで先ほどのコマンドを再度実行します。

今度は [送信データ形式] を「バイナリ」に変更し[保存]ボタンをクリック、もう一度Raspberry Piで先ほどのコマンドを再度実行します。

S3管理画面に戻り、画面上部のフォルダから一番右の数字リンク(時間)をクリックしファイル一覧を表示します。

あとから追加されたファイルを選択、ファイル形式の「JSON」を選択し[プレビュー] - [ファイルプレビュー表示]をクリックし`payload`を見ると先ほどとは中身が異なっていることが確認出来ます。

テキストの場合 : JSON内の記号はエスケープされ、単一のテキスト(文字列)になっている
```json
  "payloads": "{\"uptime\": 63913.20}",
```

バイナリの場合 : Base64形式でエンコードされている 
```json
  "payloads": "eyJ1cHRpbWUiOiA2NDYzOC43OH0=",
```

Raspberry Piで以下のコマンドでBase64エンコードされたデータをデコードできます。

```console
$ echo "eyJ1cHRpbWUiOiA2NDYzOC43OH0=" | base64 -d
{"uptime": 64638.78}
$
```
