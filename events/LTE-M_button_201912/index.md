<h1>SORACOM LTE-M Button powered by AWS ハンズオン</h1>
...
<h2 id="work-a">作業: A</h2>

* A-1. [AWS アカウントの作成](create-aws-account){:target="_blank"}
    * A-1-1. [すでに AWS アカウント持っている場合の確認事項](aws-account-available){:target="_blank"}
* A-2. [SORACOM LTE-M Button を AWS IoT 1-Click に登録し、メールを使い動作確認](claim-and-email-with-amazon-ses){:target="_blank"}
* A-3. [メールの内容を変えてみる](customize-lambda-function){:target="_blank"}
* A-4. [プレイスメントからデバイスの割り当てを外す](unassing-placement){:target="_blank"}

<h2 id="work-b">作業: B</h2>

これより先は、どの手順から開始しても構いません。それぞれで追加で必要なものがあります。

* B-1. [AWS IoT Core を使って会場にあるデバイスを制御](b-work/device-control-with-awsiotcore-shadow){:target="_blank"}
    * **追加で必要なもの**: とくに無し
* B-2. [IFTTT を使って LINE に送信](b-work/line-notify-with-ifttt){:target="_blank"}
    * **追加で必要なもの**: LINE アカウント
